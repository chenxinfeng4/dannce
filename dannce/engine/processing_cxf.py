...
def infer_params_img(params, dannce_net, prediction):
    """
    ...
    print_and_set(params, "raw_im_h", im_shape[0])
    print_and_set(params, "raw_im_w", im_shape[1])
...
def get_ft_wt(params):
    if params["dannce_finetune_weights"] is not None:
        if os.path.isfile(params["dannce_finetune_weights"]):
            weights = params["dannce_finetune_weights"]
            assert '.hdf5' in weights, "Weights file must be a hdf5 file."
            return weights
        elif os.path.isdir(params["dannce_finetune_weights"]):
...
def grab_predict_label3d_file(defaultdir=""):
    ...
    label3d_files = [
        os.path.join(def_ep, f) for f in label3d_files if "dannce" in f and ".mat" in f
    ]
    label3d_files.sort()
...
def savedata_expval(
    fname, params, write=True, data=None, num_markers=20, tcoord=True, pmax=False, otherParams=None
):
    ...
    p_max = np.array([data[ind]['pred_max'] for ind in keys])
    d_coords = np.array([data[ind]['pred_coord'] for ind in keys])
    sID = np.array([data[ind].get('sampleID', '0_0') for ind in keys])
    t_coords = np.array([np.reshape(d.get('true_coord_nogrid', np.zeros((3,num_markers))*np.nan), (3, num_markers))
                            for d in data.values()])
    com_3D = np.array([d.get('com_3d', np.zeros(3)*np.nan) for d in data.values()])
    if otherParams is None:
        otherParams = {}
    
    assert d_coords.shape == t_coords.shape and d_coords.shape[1]==3
    data_3D = d_coords.transpose(0,2,1).reshape(d_coords.shape[0], -1)
    data_3D_gt = t_coords.transpose(0,2,1).reshape(t_coords.shape[0], -1)
    sdict = {
            "data_3D": data_3D,
            "data_3D_gt": data_3D_gt,
            "com_3D": com_3D,
            "p_max": p_max,
            "sampleID": sID,
            "metadata": prepare_save_metadata(params),
