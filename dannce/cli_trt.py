...
from dannce.interface_cxf import (
    dannce_predict,
    dannce_predict_video,
    dannce_predict_video_trt,
    dannce_train,
    build_params,
)
from dannce.engine.processing_cxf import check_config, infer_params_img as infer_params
from dannce import (
...
def dannce_predict_video_cli():
    """Entrypoint for dannce prediction video."""
    parser = argparse.ArgumentParser(
        description="Dannce predict video CLI",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.set_defaults(**{**_param_defaults_shared, **_param_defaults_dannce})
    args = parse_clargs(parser, model_type="dannce", prediction=True)
    params = build_clarg_params(args, dannce_net=True, prediction=True)
    dannce_predict_video(params, args.video_file)


def dannce_predict_video_trt_cli():
    """Entrypoint for dannce prediction video."""
    parser = argparse.ArgumentParser(
        description="Dannce predict video CLI",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    ...
    args = parse_clargs(parser, model_type="dannce", prediction=True)
    params = build_clarg_params(args, dannce_net=True, prediction=True)
    dannce_predict_video_trt(params, args.video_file)
...
def add_dannce_predict_video_args(
    parser: argparse.ArgumentParser,
) -> argparse.ArgumentParser:
    
    parser.add_argument(
    "--video-file",
    help="the video path to be predicted",
    )
...
def parse_clargs(
    ...
    if model_type == "dannce":
        ...
        if prediction:
            parser = add_dannce_predict_video_args(parser)
